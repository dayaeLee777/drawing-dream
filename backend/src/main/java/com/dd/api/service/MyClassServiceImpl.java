...
//	public MyClassGetListWrapperResponseDto getMyClassList() {
		...
		for (UserDepartment ud : userDepartmentRepository
				.findBySchoolAndGradeCodeAndClassCodeOrderByUserCodeAscStudentNo(school, gradeCode, classCode).get()) {
			if (ud.isDelYn())
				continue;

			list.add(new MyClassGetListResponseDto(ud.getUser().getId(), ud.getUser().getUserName(), ud.getUserCode(),
					ud.getGradeCode(), ud.getClassCode(), ud.getStudentNo()));
		}
