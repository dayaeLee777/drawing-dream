...
public class UserController {
	...
	@ApiResponses({ @ApiResponse(code = 201, message = "회원가입이 정상적으로 완료되었습니다."),
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "회원정보가 정상적으로 수정되었습니다."),
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "비밀번호가 일치합니다."),
			@ApiResponse(code = 401, message = "인증되지 않은 사용자입니다."), })
	public ResponseEntity<? extends BaseResponseDto> checkPassword(
			@ApiIgnore @RequestHeader("Authorization") String accessToken,
			@RequestBody @ApiParam(value = "변경할 비밀번호 정보", required = true) PasswordCheckRequestDTO passwordCheckRequestDTO) {

		if (accessToken == null || !userService.checkPassword(accessToken, passwordCheckRequestDTO))
			return ResponseEntity.status(401).body(BaseResponseDto.of(401, "인증되지 않은 사용자입니다."));

		return ResponseEntity.status(200).body(BaseResponseDto.of(200, "비밀번호가 일치합니다."));

	}

	@PutMapping("/password")
	@ApiOperation(value = "비밀번호 수정")
	@ApiResponses({ @ApiResponse(code = 200, message = "비밀번호가 정상적으로 수정되었습니다."),
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "회원정보가 정상적으로 삭제되었습니다."),
			@ApiResponse(code = 401, message = "인증되지 않은 사용자입니다."),
			@ApiResponse(code = 404, message = "회원정보가 존재하지 않습니다."), })
...
//		userService.deleteUser(accessToken, userId);
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "회원 정보를 정상적으로 불러왔습니다."),
			@ApiResponse(code = 401, message = "인증되지 않은 사용자입니다."),
	...
	public ResponseEntity<? extends BaseResponseDto> getUserInfo(
		...
		if (accessToken == null)
			return ResponseEntity.status(401).body(BaseResponseDto.of(200, "인증되지 않은 사용자입니다."));
...
//			userInfoResponseDto = userService.getUserInfo(accessToken, userId);
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "회원 정보가 있습니다."),
